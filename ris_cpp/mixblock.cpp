        ...
        for (int jb = 1; jb <= mixing_data.nblock; ++jb) {
            ...
            if (nevblk > 0) {
                ...
                for (int j = 0; j < nevblk; ++j) {
                    int eigenstate_idx = nvecpat + j;  // Which eigenstate (0, 1, 2, ...)
                    
                    for (int i = 0; i < ncfblk; ++i) {
                        int csf_global_idx = ncfpat + i;  // Global CSF index across all blocks
                        int index = eigenstate_idx * mixing_data.ncftot + csf_global_idx;
                        
                        // Debug output for first few coefficients
                        if (jb <= 2 && j == 0 && i < 3) {
                            std::cout << "DEBUG: Block " << jb << ", eigenstate " << eigenstate_idx 
                                     << ", CSF " << csf_global_idx << " -> evec[" << index << "]" << std::endl;
                        }
